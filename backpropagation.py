...
class Network(object):
    ...
    def get_updated_weights(self, energy_gradient):
        ...
        for i in range(len(self.weights)):
            for j in range(len(self.weights[i])):
                updated_weights[i, j] = self.weights[i, j] - \
                    self.ni*energy_gradient[i, j]
    ...
    def show_energy_diff_plots(partial_energies, whole_energies):
        fig, (ax0, ax1) = plt.subplots(
            2, 1, constrained_layout=True, figsize=(9, 8))
        iter_p = range(len(partial_energies))
        ax0.set_title('Zmiana wartość energii w trybie energii cząstkowej')
        ax0.plot(iter_p, partial_energies[:, 0],
                 label='Energia dla wektora [1,0,0]')
        ax0.plot(iter_p, partial_energies[:, 1],
                 label='Energia dla wektora [1,0,1]')
        ax0.plot(iter_p, partial_energies[:, 2],
                 label='Energia dla wektora [1,1,0]')
        ax0.plot(iter_p, partial_energies[:, 3],
                 label='Energia dla wektora [1,1,1]')
        ax0.legend(loc='upper right')
        ax0.set_ylabel('Energia')
        ax1.set_xlabel('Nr iteracji')
        iter_w = range(len(whole_energies))
        ax1.plot(iter_w, whole_energies[:, 0],
                 label='Energia dla wektora [1,0,0]')
        ax1.plot(iter_w,  whole_energies[:, 1],
                 label='Energia dla wektora [1,0,1]')
        ax1.plot(iter_w,  whole_energies[:, 2],
                 label='Energia dla wektora [1,1,0]')
        ax1.plot(iter_w,  whole_energies[:, 3],
                 label='Energia dla wektora [1,1,1]')
    ...
    def solve(self):
        ...
        while self.is_gradient_changing(weights_diff) and iteration < 20000:
            for i in range(len(self.input_vectors)):
                [partial_energy, output_vectors] = self.cycle(
                    self.input_vectors[i], self.dest[i])
                energy_gradient = self.gradient_matrix(
                    self.dest[i], output_vectors)
    ...
    def solve(self):
        ...
        while self.is_gradient_changing(weights_diff) and iteration < 20000:
            ...
            for i in range(len(self.input_vectors)):
                [partial_energy, output_vectors] = self.cycle(
                    self.input_vectors[i], self.dest[i])
                energy_gradient_sum += self.gradient_matrix(
                    self.dest[i], output_vectors)
                energy[i] = partial_energy
                solution[i] = output_vectors[0]
...
def main():
    weights_init = np.array(
        [[0.86, -0.16, 0.28], [0.82, -0.51, -0.89], [0.04, -0.43, 0.48]], np.double)
